...
## Workflow
- Push to main directly after each commit; always pull `--ff-only` before pushing (pre-flight)
- One logical change per commit; Conventional Commits with scope (pre-flight)
- Maintain the dual changelog + devlog system for every behavior-changing commit (pre-flight)

## Communication
- Plain-language public changelog (grandmother test); technical detail goes in reports and the devlog (pre-flight)

## Code style
- Comment the why, not the what; prefer DRY shared helpers; match surrounding conventions (pre-flight)

## Review depth
- Fix safe issues autonomously; flag architectural changes for approval rather than implementing (pre-flight)

## Risk & approvals
- Never bump a major version or declare a production release without flagging it in the report (pre-flight)
- Schema/database changes need explicit approval before applying (pre-flight)
