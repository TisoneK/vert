...
| ZCode | qwen3.8-flash | 2026-09-15 | 2026-09-15 | 2 |
...
- **ZCode / qwen3.8-flash:** Two sessions on 2026-09-15 (core 0.8.0→1.1.3 major migration + office close; then the Vercel deploy-outage investigation, as Ada/S002). Model id stated in system prompt, recorded verbatim. Demonstrated: remote-service outage diagnosis with no service credentials — public GitHub status/check-run APIs to bisect deploy failures across commits, upstream issue archaeology to root-cause a framework regression (vercel/next.js#96646), and a fix verified on the real builder via preview deploy before pushing to main. Owner authorized the Vercel CLI device flow (`npx vercel login <email>` → user clicks a device URL) mid-session — the agent correctly stopped and asked rather than hunting the filesystem for credentials. (2026-09-15)
