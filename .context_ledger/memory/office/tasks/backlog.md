...
| B-2026-09-15-18 | **Lockfile-drift discipline for dependency sweeps** — the 2026-09-02 security sweep's lock regeneration silently moved `next` 16.2.9→16.3.4 under a `^16.2.11` range, crossing into a version line with a Vercel-builder regression that took production deploys down for 13 days (fixed by 0.9.2's 16.3.5 bump). Adopt a rule for future sweeps: after regenerating the lock, diff it for MINOR/MAJOR version moves and either justify each in the commit body or re-resolve pinned to the intended range; after any sweep touching the toolchain, confirm the production deploy actually went green (`vercel ls` / commit status) before closing the session. |
