...
Friction caused by **the `.context_ledger/` system or the protocol itself** —
...
| The protocol steps, the `.context_ledger/` rules, a template, a missing rule | `flaws/log.md` (this file) |
...
2. **`.context_ledger/`** — agent memory. Commits use `chore(ledger):`. Friction
   with the `.context_ledger/` system or the protocol goes in `flaws/log.md`
   (this file).
